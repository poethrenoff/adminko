...
class BlockTable extends BuilderTable
{
    ...
    protected function action_param_save($redirect = true)
    {
        ...
        {
            $errors = array();
            if ($param_value['param_require']) $errors[] = 'require';
    ...
    protected function action_multiply($redirect = true)
    {
        ...
        $insert_block_params = Db::selectAll('select param, value from block_param where block = :block_id',
            array('block_id' => $record['block_id']));
        
        foreach ($page_list as $page)
        {
            $block = Db::selectRow('select * from block where block_page = :block_page and block_area = :block_area',
